...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    ...
    .then(printPizza)
    // if error, bring user back to homepage
    .catch((err) => {
      console.log(err);
...
function handleNewCommentSubmit(event) {
  ...
  // get Comment values from form data
  ...
  // if Comment is missing any data, return false
  ...
  // when at pizza discussion URL endpoint
  fetch(`/api/comments/${pizzaId}`, {
    ...
    // stringify JSON comment-form data
...
function handleNewReplySubmit(event) {
  ...
  // if they're missing required data, return false
